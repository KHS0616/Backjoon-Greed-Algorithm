...
센서들은 이 직선 위의 한 기점인 원점으로부터의 정수 거리의 위치에 놓여 있다고 하자. 따라서, 
각 센서의 좌표는 정수 하나로 표현된다. 
이 상황에서 각 집중국의 수신 가능영역의 거리의 합의 최솟값을 구하는 프로그램을 작성하시오. 
단, 집중국의 수신 가능영역의 길이는 0 이상이며 모든 센서의 좌표가 다를 필요는 없다.
"""

# 센서의 개수 N, 집중국의 개수 K, 센서들의 좌표 리스트 S를 입력받는다.
N, K = int(input()), int(input())
S = list(map(int, input().split()))

# 센서들의 좌표를 오름차순으로 정렬한다.
# 그 후, 각 센서간의 거리를 구하고 리스트 L에 저장한다.
S.sort()
L = []
for i in range(N-1):
    L.append(S[i+1] - S[i])

# 집중국의 개수만큼 센서들을 묶을 수 있다.
# 즉, 센서사이의 길이의 요소들 중에서 K-1 개 만큼 제거가능
# 최대길이의 K-1 개를 제거한 후 나머지들의 합을 구한다.
# 단, 센서의 개수가 집중국의 개수보다 작거나 같을 경우 길이는 0이된다.
if not N <= K:
    for i in range(K-1):
        L.remove(max(L))
    print(sum(L))
